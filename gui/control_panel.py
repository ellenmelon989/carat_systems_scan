...
Operator-facing scan parameters plus Start/Abort. Exposes exactly the
two knobs scan_params.py validates (dwell time, step size), plus scan
order and output directory. Grid range (x_range_mm/y_range_mm) is
deliberately NOT an editable field here -- it's calibration-only, set
by calibrate_scan_area.py, matching the existing "operator sets step
size, range comes from calibration" split already documented in
scan_params.py.
"""
...
from scan_params import (
    ...
    validate_dwell_time_s, validate_step_size_mm,
)
...
class ControlPanel(ttk.Frame):
    ...
    def _handle_start(self):
        ...
        # (ScanManager.__init__ calls validate_dwell_time_s again too) --
        # this just gives the operator an immediate error dialog instead
        # of a scan that starts and then blows up on ScanManager
        # construction inside the worker thread a moment later.
        try:
